...
const Comment = memo(({ comment, formatDate, isPinned = false }) => (
    <div
        className={`px-4 pt-4 pb-2 rounded-xl border transition-all group hover:shadow-lg hover:-translate-y-0.5 ${
            isPinned 
                ? 'bg-gradient-to-r from-zinc-500/10 to-zinc-700/10 border-zinc-500/30 hover:bg-gradient-to-r hover:from-zinc-500/15 hover:to-zinc-700/15' 
    ...
    >
        ...
        <div className="flex items-start gap-3">
            {comment.profile_image ? (
                <img
                    ...
                    className={`w-10 h-10 rounded-full object-cover border-2 flex-shrink-0  ${
                        isPinned ? 'border-zinc-500/50' : 'border-zinc-500/30'
                    }`}
                    loading="lazy"
                />
            ) : (
                <div className={`p-2 rounded-full text-zinc-400 group-hover:bg-zinc-500/30 transition-colors ${
                    isPinned ? 'bg-zinc-500/30' : 'bg-zinc-500/20'
                }`}>
            ...
            <div className="flex-grow min-w-0">
                <div className="flex items-center justify-between gap-4 mb-2">
                    <div className="flex items-center gap-2">
                        <h4 className={`font-medium truncate ${
                            isPinned ? 'text-zinc-200' : 'text-zinc-100'
                        }`}>
                            {comment.user_name}
                        </h4>
                        {isPinned && (
                            <span className="px-2 py-0.5 text-xs bg-zinc-500/20 text-zinc-300 rounded-full">
                                Admin
                            </span>
                        )}
                    </div>
                ...
                <p className="text-zinc-400 text-sm break-words leading-relaxed relative bottom-2">
                    {comment.content}
                </p>
            </div>
        </div>
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {imagePreview ? (
                        <div className="flex items-center gap-4">
                            <img
                                src={imagePreview}
                                alt="Profile preview"
                                className="w-16 h-16 rounded-full object-cover border-2 border-zinc-500/50"
                            />
                            <button
                                type="button"
                                onClick={() => {
            ...
            <button
                ...
                className="relative w-full h-12 bg-zinc-100 rounded-xl font-medium text-zinc-900 overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed"
            >
...
const Komentar = () => {
    ...
    return (
        <div className="w-full bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl shadow-xl" data-aos="fade-up" data-aos-duration="1000">
            <div className="p-6 border-b border-white/10" data-aos="fade-down" data-aos-duration="800">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-semibold text-zinc-100">
                        Comments <span className="text-zinc-400">({totalComments})</span>
                    </h3>
                </div>
            </div>
